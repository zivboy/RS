...
requirejs(['jquery', 'bootstrap', 'table', 'tablezn', 'tExport', 'tExportS', 'base64', 'comm', 'message'],
    function () {
        ...
        var $table = $('#tableB').bootstrapTable({
            url: WEB_GLOBAL_CTX + '/business/model/list',
            dataType: 'json',
            cache:false,
            showToggle:true,
            showExport:true,
        ...
        $('#delete').click(function () {
            ...
            $.each(objects, function () {
                $.post(WEB_GLOBAL_CTX + "/business/model/delete/" + this.modelId, function (rsp) {
                    if (rsp.successful) {
                        $.scojs_message(rsp.msg, $OK);
                        flashTable('tableB', '/business/model/list');
                    } else {
        ...
        $('#add').click(function () {
            ...
            self.location = WEB_GLOBAL_CTX + "/business/model/add";
        });
        ...
        $('#modify').click(function () {
            ...
            var objects = $table.bootstrapTable('getSelections');
            $.each(objects, function () {
                self.location = WEB_GLOBAL_CTX + "/business/model/edit/"+this.modelId;
            });
        });

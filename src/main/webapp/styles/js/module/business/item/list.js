...
requirejs(['jquery', 'bootstrap', 'table', 'tablezn', 'tExport', 'tExportS', 'base64', 'comm', 'message'],
    function () {
        ...
        var $table = $('#tableB').bootstrapTable({
            url: WEB_GLOBAL_CTX + '/business/item/list',
            dataType: 'json',
            cache:false,
            showToggle:true,
            showExport:true,
        ...
        $('#delete').click(function () {
            ...
            $.each(objects, function () {
                $.post(WEB_GLOBAL_CTX + "/business/item/delete/" + this.itemId, function (rsp) {
                    if (rsp.successful) {
                        $.scojs_message(rsp.msg, $OK);
                        flashTable('tableB', '/business/item/list');
                    } else {
        ...
        $('#add').click(function () {
            ...
            self.location = WEB_GLOBAL_CTX + "/business/item/add";
        });
        ...
        $('#modify').click(function () {
            ...
            var objects = $table.bootstrapTable('getSelections');
            $.each(objects, function () {
                self.location = WEB_GLOBAL_CTX + "/business/item/edit/"+this.itemId;
            });
        });
